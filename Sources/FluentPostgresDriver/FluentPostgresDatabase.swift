        ...
        if case .read = query.action {
            ...
            for (i, f) in query.fields.enumerated() {
                ...
                if case let .extendedPath(path, schema, space, alias) = f {
                    if self.automaticallyAliasIdentiferOverflow {
                        let aliasString = (space.map { "\($0)_" } ?? "") + schema + "_" + key(path[0])
                        if (aliasString.count > 33) {
    //                        let overflowAlias = "\(UUID())_\(key(path[0]).prefix(26))"
                            let aliasHash = abs(aliasString.hash)
                            let overflowAlias = "al_ov_\(aliasHash)"
                            //queryFinal.fields[i] = .extendedPath(path, schema: schema, space: space, alias: overflowAlias)
                            let key = key(path[0])
                            queryFinal.fields[i] = .custom(SQLAlias(SQLColumn(key, table: schema), as: overflowAlias))
        ...
        // REID - Custom code ends
            
        var expression = SQLQueryConverter(delegate: PostgresConverterDelegate()).convert(queryFinal)
...
////        self.logger.log(level: .debug, "\(sql) \(binds)")
//        do {
...
//        }
////        
        return self.execute(sql: expression, { onOutput($0.databaseOutput(columnOverflowAliasDict: columnOverflowAliasDict)) })
    }
